...
static float __EaseOutQuad(float t)
{
	return 1 - (1 - t) * (1 - t);
}

void CPythonMiniMap::__StartZoomProcess()
{
	m_dwZoomBeginTime = CTimer::Instance().GetCurrentMillisecond();
	m_dwZoomEndTime = m_dwZoomBeginTime + g_dwMinimapSmoothZoomMSec;
}

bool CPythonMiniMap::__IsZoomProcess() const
{
	return m_fTargetZoom != m_fScale;
}
#endif
...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	if (__IsZoomProcess())
	{
		const long lCurMSec = CTimer::Instance().GetCurrentMillisecond();
		
		if (lCurMSec <= m_dwZoomEndTime)
		{
			const float fRatio = static_cast<float>(lCurMSec - m_dwZoomBeginTime) / g_dwMinimapSmoothZoomMSec;
			SetScale(GetLinearInterpolation(m_fScale, m_fTargetZoom, __EaseOutQuad(fRatio)));
		}
		else
		{
			SetScale(m_fTargetZoom);
		}
	}
#endif
...
#ifdef ENABLE_RENEWAL_OFFLINESHOP
			if (pkInstEach->IsShop())
				m_OfflineShopPositionVector.push_back(aMarkPosition);
			else
...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
	...
	while (aIterator != m_OfflineShopPositionVector.end())
	{
		TMarkPosition & rPosition = *aIterator;
		m_WhiteMark.SetPosition(rPosition.m_fX, rPosition.m_fY);
		m_WhiteMark.Render();
	...
	if (pkCmrCur)
	{
		...
		const float fAlpha = 0.3f;
		const float fRadius = 128.0f / 2.0f;
...
void CPythonMiniMap::ScaleUp()
{
	...
	m_fTargetZoom = fMIN(m_fTargetZoom * 2.0f, 4.0f);
	__StartZoomProcess();
}
...
void CPythonMiniMap::ScaleDown()
{
	...
	m_fTargetZoom = fMAX(m_fTargetZoom * 0.5f, 0.5f);
	__StartZoomProcess();
}
#else
void CPythonMiniMap::ScaleUp()
...
#ifdef ENABLE_RENEWAL_MINIMAP_MARK
...
void CPythonMiniMap::AddPartyPositionInfo(const TPartyPosition& Info)
{
	...
	if (it == mAtlasPartyPlayerMark.end())
	{
		...
		PartyInfo->sName = pPartyMemberInfo->strName;
