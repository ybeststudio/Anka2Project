...
BOOL CActorInstance::TestActorCollision(CActorInstance & rVictim)
{
	...
	if (rVictim.GetRace()>=34001 && rVictim.GetRace()<=34164)
		return FALSE;
#endif

#ifdef REMOVE_SHOPS_COLLISION
	if (rVictim.IsShop())
		return FALSE;
#endif

#ifdef REMOVE_MOUNTS_COLLISION
	if (rVictim.GetRace()>=20101 && rVictim.GetRace()<=20299)
		return FALSE;
#endif

#ifdef REMOVE_OX_PLAYER_COLLISION
...
void CActorInstance::BlockMovement()
{
	if (m_pkHorse)
	{
		...
		// Standing mount ise karakter de hareket etmemeli
		DWORD dwHorseRace = m_pkHorse->GetRace();
		if (dwHorseRace == STANDING_MOUNT_VNUM_1 || 
			dwHorseRace == STANDING_MOUNT_VNUM_2 || 
			dwHorseRace == STANDING_MOUNT_VNUM_3)
		{
